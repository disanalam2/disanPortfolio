...
const About = ({ isAdmin }) => {
  ...
  useEffect(() => {
    const fetchAboutData = async () => {
      try {
        const response = await fetch('https://d3sh63r9ecih9a.cloudfront.net/api/about');
        if (response.ok) {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch('https://d3sh63r9ecih9a.cloudfront.net/api/about/update', {
        method: 'PUT',
        headers: {
